...
from loop_tool_service.models.llvm_autotuning.autotuners.greedy import greedy  # noqa autotuner
from loop_tool_service.models.llvm_autotuning.autotuners.nevergrad_ import nevergrad  # noqa autotuner
from loop_tool_service.models.llvm_autotuning.autotuners.opentuner_ import opentuner_ga  # noqa autotuner
from loop_tool_service.models.llvm_autotuning.autotuners.random_ import random  # noqa autotuner
from loop_tool_service.models.llvm_autotuning.optimization_target import OptimizationTarget
...
class Autotuner(BaseModel):
    ...
    def __call__(self, env: CompilerEnv, seed: int = 0xCC) -> CompilerEnvState:
        ...
        print("\n\n****************** ENDING ******************\n\n")
        applied_acitons = [ env.action_space.to_string(a) for a in env.actions]
        ces = CompilerEnvState(
            benchmark=env.benchmark.uri,
            commandline=", ".join(applied_acitons),#env.commandline(),
            walltime=timer.time,
