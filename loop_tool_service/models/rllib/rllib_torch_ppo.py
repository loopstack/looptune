...
def run_benchmarks(train_benchmarks):
    breakpoint()
    ...
    with make_training_env() as env:
        for i, bench in enumerate(env.datasets.benchmarks()):
            if i == 4: break
            print(f">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>{bench}")
...
def run_agent_on_benchmarks(policy_model, benchmarks, bench_num=20):
    """Run agent on a list of benchmarks and return a list of cumulative rewards."""
    chosen_benchmarks = random.sample(benchmarks.dataset, bench_num)
    with make_env() as env:
        df_gflops = pd.DataFrame(np.zeros((len(chosen_benchmarks), 4)), columns=['bench', 'base', 'network', 'search'])

        for i, benchmark in enumerate(chosen_benchmarks, start=0):
            ...
            print(f"[{i}/{len(chosen_benchmarks)}] ")
...
def plot_results(df_gflops_train, df_gflops_val, wandb_run_id=None):
    ...
    fig, axs = plt.subplots(1, 2, figsize=(40, 5), gridspec_kw={'width_ratios': [len(df_gflops_train) / len(df_gflops_val), 1]})
    ...
    if 'fcnet_hiddens' in default_config['model']:
        wandb_log['layers_num'] = len(default_config['model']['fcnet_hiddens'])
        wandb_log['layers_width'] = default_config['model']['fcnet_hiddens'][0]
...
if __name__ == '__main__':
    ...
    if args.slurm:
        ...
        ray.init(address=ray_address, _node_ip_address=head_node_ip, _redis_password=redis_password, log_to_driver=False)    
    else:
        ray.init(local_mode=args.local_mode, ignore_reinit_error=True, log_to_driver=False)
